 ...
 * чтобы вычислить путь от начала до конца с помощью алгоритма поиска пути A*.
...
public class AStarApp {
     ...
     * изменяя ячейки на основе состояния кнопки мыши и первоначального редактирования,
    ...
    private class MapCellHandler implements MouseListener {
         ...
         * Что это такое, зависит от исходного состояния ячейки, в которой была начата операция.
         **/
    ...
    /** Создает новый экземпляр AStarApp с заданными шириной и высотой карты. **/
     ...
     * Это вызывается из потока обработчика событий Swing, чтобы быть threadsafe.
     **/
     ...
     * используя текущее состояние карты. Реализация происходит довольно медленно;
     * создается новый объект {@link Map2D}, который инициализируется из текущего состояния приложения.
     * Затем вызывается навигатор A*, и если путь найден, то дисплей обновляется,
    ...
    /** Точка входа для приложения. В настоящее время никакие аргументы командной строки не распознаются. **/
